...
***Designing Login Form using Linear Layout***

To design Login form using Linear Layout.

- Create new android project using Android Studio and name it as LoginForm

- From the menu bar select File -> XML -> Create LayoutXML

- Design the login form layout as follows:

  - LinearLayout(Horizontal, fill_parent, fill_parent)
    
      - LinearLayout(Vertical, fill_parent, fill_parent)
      
        - TextView (wrap_content, wrap_content)
        
        - LinearLayout(Horizontal, fill_parent, wrap_content)
          
          - TextView(wrap content, wrap content, username)  
          
          - EditText(200 dp, wrap content)
          
        - LinearLayout(Horizontal, fill_parent, wrap_content)
          
          - TextView(wrap content, wrap content, password)  
          
          - EditText(200 dp, wrap content)          
        
        - Button(wrap_content, wrap_content)  
...
```{r}
knitr::include_url('https://rpubs.com/laxmikant/relativelayout')
```
