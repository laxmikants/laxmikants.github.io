...
```{python,echo = FALSE}
df = pd.read_excel('silverprices.xlsx')
...
# Define the URL for MCX Silver Micro data
url = "https://www.mcxindia.com/market-data/bhavcopy"

# Send a GET request to the URL
response = scraper.get(url)

# Extract the table data from the response using pandas
tables = pd.read_html(response.content)

# Get the required table with MCX Silver Micro data
mcx_data = tables[0]  # Adjust the index as per the actual table position
...
# Define the commodity code for MCX Silver Micro
commodity_code = "MCX/SILVERMICRO"

# Set the desired date range
from_date = "2023-01-01"
to_date = "2023-06-27"

# Fetch the historical data for MCX Silver Micro
silver_micro_data = quandl.get(commodity_code, start_date=from_date, end_date=to_date)

# Print the retrieved data
print(silver_micro_data)
```

```{python,eval=FALSE,echo=FALSE}
import yfinance as yf

# Define the ticker symbol for MCX Silver
symbol = "SILVER.NS"

# Fetch the historical data for MCX Silver
silver_data = yf.download(symbol, start=from_date, end=to_date)


# Print the retrieved data with price per kilogram
print(silver_data)
```
...
# Define the ticker symbol for silver in USD
symbol = "SI=F"


# Fetch the historical data for silver in USD
dfusd = yf.download(symbol, start=from_date, end=to_date)
...
#usdinr = investpy.get_currency_cross_historical_data(currency_cross='USD/INR', from_date='09/10/2019', to_date='19/05/2022')

#dfusd = investpy.get_commodity_historical_data(commodity='Silver', from_date='09/10/2019', to_date='19/05/2022')

#df = investpy.commodities.get_commodity_historical_data(commodity='MCX Silver Micro',from_date='09/10/2019',to_date='19/05/2022', country = "India")

dfs = df.copy()

#info = investpy.commodities.get_commodity_information(commodity='MCX Silver Micro', country = "India",as_json=True)

#print(info['Todays Range'])
```


***1.2.1: Determining position based on volume and close***

```{python, echo = FALSE, warning = FALSE, message = FALSE, eval=FALSE}
...
short = df2[df2['Open'] == df2['High']]
lng = df2[df2['Open'] == df2['Low']]


short['gain'] = short.iloc['High'] - short.iloc['Close']
lng['gain'] = lng.iloc['Close'] - lng.iloc['Low']
...
data1 = investpy.moving_averages(name='MCX Silver Micro', country='India', product_type='commodity', interval='1hour')
#print(data1.head())
...
df['300d_ma'] = round(df[ 'Close' ].rolling(window = 300, min_periods = 0).mean(),2)
...
df['300d_ema'] = round(df[ 'Close' ].rolling(window = 300, min_periods = 0).mean(),2)
...
STOK = ((MACD - MACD.rolling(window=10).min()) / (
...
print(round(upper[-1],2)*1000,round(middle[-1],2)*1000,round(lower[-1],2)*1000)
...
df_ret.drop(df_ret.index[:16], inplace=True)
...
X['usdinr'] = df['usdinr'].copy()

X['usdsilver'] = df['usdsilver'].copy() 

X["20d_ma"] = df['20d_ma' ].copy()
X["20d_ema"] = df['20d_ema' ].copy()

X["50d_ma"] =  df['50d_ma'].copy()
X["50d_ema"] = df['50d_ema' ].copy()

X["100d_ma"] =  df['100d_ma'].copy()
X["100d_ema"] =  df['100d_ema'].copy()

X["200d_ma"] =  df['200d_ma'].copy()
X["200d_ema"] =  df['200d_ema'].copy()


X['250d_ma'] = df['250d_ma'].copy()
X['250d_ema'] = df['250d_ema'].copy()


X['Std_dev'] = df['Std_dev'].copy()

X['RSI'] = df['RSI'].copy()

X['Williams %R'] = df['Williams %R'].copy()

X['MACD_IND'] = df['MACD_IND'].copy()

X['VolChange'] = df['VolChange'].copy()

X['CloseChange'] = df['CloseChange'].copy() 

X['zscore'] = df['zscore'].copy() 
...
X['Open-Close']  = df['Open-Close'].copy() 

X['Open-Open'] = df['Open-Open'].copy() 
...
#s1 = data2.iloc[1,1].astype(int)
#s2 = data2.iloc[1,2].astype(int)
#s3 = data2.iloc[1,3].astype(int)
#pp = data2.iloc[1,4].astype(int)
#r1 = data2.iloc[1,5].astype(int)
#r2 = data2.iloc[1,6].astype(int)
#r3 = data2.iloc[1,7].astype(int)

#lclose = dfs.iloc[-1]['Close'].astype(int)
...
print('Hourly Technical Indicators:')

data2_hr = investpy.pivot_points(name='MCX Silver Micro', country='India', product_type='commodity', interval='1hour')
print(data2_hr.head())

data3_hr = investpy.technical_indicators(name='MCX Silver Micro', country='India', product_type='commodity', interval='1hour')
print(data3_hr.head())

data1_hr = investpy.moving_averages(name='MCX Silver Micro', country='India', product_type='commodity', interval='1hour')
print(data1_hr.head())

print('Daily Technical Indicators:')

data2_dl = investpy.pivot_points(name='MCX Silver Micro', country='India', product_type='commodity', interval='daily')
print(data2_dl.head())

data3_dl = investpy.technical_indicators(name='MCX Silver Micro', country='India', product_type='commodity', interval='daily')
print(data3_dl.head())

data1_dl = investpy.moving_averages(name='MCX Silver Micro', country='India', product_type='commodity', interval='daily')
print(data1_dl.head())


print(X.iloc[-1])    

if(lclose < data2_dl.iloc[1,4]):
  print("Bearish daily: Sell at ",data2_dl.iloc[1,3])
else:
  print("Bullish at daily: Buy at ", data2_dl.iloc[1,5])
  
if(lclose > data2_hr.iloc[1,4]):
  print("Bullish hourly: Buy at ", data2_hr.iloc[1,5])
else:
  print("Bearish hourly: Sell at ", data2_hr.iloc[1,3])
  

if(predict == 1):
    print('Pridiction: Buy at ', data2_dl.iloc[1,5])
else:
    print('Prediction: Sell at : ', data2_dl.iloc[1,4])

if(dfs.iloc[-1]['Open'] == dfs.iloc[-1]['Low']): 
  print('Open Low Strategy ', 'Buy Now')

if(dfs.iloc[-1]['Open'] == dfs.iloc[-1]['High']): 
  print('Open High Strategy ', 'Sell now' )
...
usdinr = investpy.get_currency_cross_historical_data(currency_cross='USD/INR', from_date='01/01/2015', to_date='06/11/2021')

df = investpy.get_commodity_historical_data(commodity='Silver', from_date='01/01/2015', to_date='06/11/2021')

#info = investpy.commodities.get_commodity_information(commodity='Silver',as_json=True)
