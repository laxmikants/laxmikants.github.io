...
df = investpy.commodities.get_commodity_historical_data(commodity='MCX Silver',from_date='01/12/2019',to_date='18/09/2020', country = "India")
...
df[ '5d_ma' ] = round(df[ 'Close' ].rolling( window = 7 , min_periods = 0 ).mean(),2)
df[ '20d_ma' ] = round(df[ 'Close' ].rolling( window = 21 , min_periods = 0 ).mean(),2)
df['50d_ma'] = round(df[ 'Close' ].rolling(window = 49, min_periods = 0).mean(),2)
df['100d_ma'] = round(df[ 'Close' ].rolling(window = 98, min_periods = 0).mean(),2)
df['200d_ma'] = round(df[ 'Close' ].rolling(window = 196, min_periods = 0).mean(),2)
...
```{python, warnings = FALSE, message = FALSE}
...
daily_silver = Quandl("MCX/SIU2020",collapse="daily",start_date="2020-01-01",end_date = "2020-09-03",type="raw")
